<div x-data="{ 
        ...
        initChat() {
            ...
            // Note: Replace localhost:8787 with your deployed Worker URL
            this.ws = new WebSocket('ws://localhost:8787');
            
            this.ws.onopen = () => {
                console.log('🔌 Connected to Cloudflare WS');
                // Send 'user-online' signal? 
                this.ws.send(JSON.stringify({ signal: 'user-online', user: {{ auth()->id() }} }));
            };
            ...
            this.ws.onmessage = (event) => {
                try {
                    ...
                    // Presence logic for the specific user we are chatting with
                    const otherUserId = {{ $user->id }};
                    
                    if (data.signal === 'online-users-list') {
                        this.isOnline = data.users.includes(otherUserId);
                    }
                    else if (data.signal === 'user-online' && data.user === otherUserId) {
                        this.isOnline = true;
                    }
                    else if (data.signal === 'user-offline' && data.user === otherUserId) {
                        this.isOnline = false;
                    }
    ...
    }" x-init="initChat()" @message-sent-to-db.window="signalNewMessage(); scrollToBottom()"
    ...
    <div id="messages" class="flex-1 overflow-y-auto p-4 bg-gray-50 flex flex-col gap-3 scroll-smooth">
        @forelse ($messages as $message)
            <div class="flex w-full {{ $message->from_user_id == auth()->id() ? 'justify-end' : 'justify-start' }}">
                <div
                    class="flex max-w-[80%] md:max-w-[70%] gap-2 {{ $message->from_user_id == auth()->id() ? 'flex-row-reverse' : 'flex-row' }}">

                    {{-- Avatar Pequeño en mensaje --}}
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($message->sender->name) }}&color={{ $message->from_user_id == auth()->id() ? '7F9CF5' : 'CBD5E0' }}&background={{ $message->from_user_id == auth()->id() ? 'EBF4FF' : 'F7FAFC' }}&size=32"
                        alt="{{ $message->sender->name }}" class="h-8 w-8 rounded-full self-end mb-1 shadow-sm">

                    <div
                        class="px-4 py-2 rounded-2xl shadow-sm relative group {{ $message->from_user_id == auth()->id() ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-white text-gray-800 border border-gray-100 rounded-bl-sm' }}">
    ...
    <div class="bg-white p-4 border-t border-gray-200">
        <form wire:submit.prevent="sendMessage" class="flex items-center gap-2">
            ...
            <button type="submit"
                ...
                wire:loading.attr="disabled" @click="setTimeout(() => scrollToBottom(), 100)">
                <svg wire:loading.remove class="w-5 h-5 transform rotate-90" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
