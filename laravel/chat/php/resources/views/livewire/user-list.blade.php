<div x-data="{
        ...
        ws: null,
        init() {
            console.log('🌍 Connecting to Cloudflare WS for presence');
            ...
            this.ws.onmessage = (event) => {
                try {
                    ...
                    if (data.signal === 'online-users-list') {
                        // Initial full list
                        this.onlineUserIds = data.users;
                        console.log('👥 Online Users:', this.onlineUserIds);

                    } else if (data.signal === 'user-online') {
                        // User came online
                        if (!this.onlineUserIds.includes(data.user)) {
                            this.onlineUserIds.push(data.user);
                        }
                    } else if (data.signal === 'user-offline') {
                        // User went offline
                        this.onlineUserIds = this.onlineUserIds.filter(id => id !== data.user);
                    }
                } catch(e) {
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-4">
                            <div class="relative">
                                ...
                                <span x-show="isUserOnline({{ $user->id }})"
                                    ...
                                    x-transition:leave-end="transform opacity-0 scale-50"></span>
                            </div>

                            <div>
                                <span
                                    class="text-lg font-semibold text-gray-800 group-hover:text-blue-600 transition">{{ $user->name }}</span>
