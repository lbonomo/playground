...
export default {
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        // Handle WebSocket upgrade or Broadcast API
        // We map everything to a single Durable Object instance for this demo (Singleton pattern)
        // in a real app, you might shard by room ID, user ID, etc.
        if (url.pathname === "/websocket" || url.pathname === "/broadcast") {
            const id = env.WEBSOCKET_SOURCE.idFromName("GLOBAL_BROADCAST_ROOM");
            const stub = env.WEBSOCKET_SOURCE.get(id);
